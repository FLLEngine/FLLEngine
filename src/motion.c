...
void motion_init(int motor1Port, int attachment1Port, float wheelDiameter, bool invert) {
    ...
    writeFileStr(attachmentMotors.attachment1.command, "reset");
    writeFileStr(attachmentMotors.attachment2.command, "reset");
}
...
float *toLoc(float targetLoc[], float currentLoc[], int speed) {
    float lineY = targetLoc[0]-currentLoc[0];
    float lineX = targetLoc[1]-currentLoc[1];
    float targetAngle = (atan2(lineX, lineY))*-57.29578;  //strange number is radians to degrees conversion
    float distance = sqrt(lineX*lineX+lineY*lineY);
    if(targetAngle<0){
        targetAngle = 360+targetAngle;
    }

    printf("from: (%f, %f) | To: (%f, %f) | Speed: %d | Angle: %f, %f, %f\n", currentLoc[0], currentLoc[1], targetLoc[0], targetLoc[1], speed, targetAngle, lineX, lineY);

    turnAngle(targetAngle);
    driveStraight(distance, targetAngle, speed);
...
void driveStraight(int distance, float angle, int speed) {
    ...
    while( ((double)((readFileInt32(drivingMotors.Motor1.positionRD)*robot.motorMult + readFileInt32(drivingMotors.Motor2.positionRD)*robot.motorMult)/2-startingDistance) < adjDistance && (double)((readFileInt32(drivingMotors.Motor1.positionRD)*robot.motorMult + readFileInt32(drivingMotors.Motor2.positionRD)*robot.motorMult)/2-startingDistance) > 0-adjDistance)) {  //compares average of motor positions to target distance
        ...
        writeFileInt(drivingMotors.Motor1.speedSP, (-speed+((error*3)))*robot.motorMult);
        writeFileInt(drivingMotors.Motor2.speedSP, (-speed+(0-(error*3)))*robot.motorMult);
        writeFileStr2(drivingMotors.Motor1.command, drivingMotors.Motor2.command, "run-forever");
    }
    writeFileStr2(drivingMotors.Motor1.stopAction, drivingMotors.Motor2.stopAction, "hold");
    writeFileStr2(drivingMotors.Motor1.command, drivingMotors.Motor2.command, "stop");
}
